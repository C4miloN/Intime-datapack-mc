...
execute if entity @p[distance=..64,scores={difficulty=3..}] if predicate pdt:chance_20 run function pdt:mobs/zombie_minigiant
execute if entity @p[distance=..64,scores={difficulty=5..}] if predicate pdt:chance_05 run function pdt:mobs/zombie_chronos
execute if entity @p[distance=..64,scores={difficulty=5..}] run effect give @s resistance infinite 2 false